...
from dataset.cifar10 import cifar10, cifar10_c, cifar10_plus
...
def parse_data(name, batch_size, valid_ratio = None, shuffle = True, augmentation = True, **kwargs):
    ...
    if name.lower() in ['cifar10',]:
        ...
        train_loader, valid_loader, test_loader, classes = svhn(batch_size = batch_size, valid_ratio = valid_ratio, shuffle = shuffle, **kwargs)
    else:
        raise ValueError('Unrecognized name of the dataset: %s' % name)

    return train_loader, valid_loader, test_loader, classes

def parse_c_data(name, batch_size, c_type, level, **kwargs):

    level = int(level)

    if name.lower() in ['cifar10',]:
        _, _, test_loader, classes = cifar10_c(batch_size = batch_size, c_type = c_type, level = level)
    else:
        raise ValueError('Unrecognized name of the dataset: %s' % name)
