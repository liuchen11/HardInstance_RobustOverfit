...
from util.train import train_instancewise
...
if __name__ == '__main__':
    ...
    parser.add_argument('--warmup_epoch', type = int, default = 5,
        help = 'The length of the warmup epoch phase, default is 5.')
    ...
    parser.add_argument('--beta', type = float, default = 0.1,
        help = 'The momentum of the threshold calculation, default is 0.1.')
    parser.add_argument('--gamma', type = float, default = 0.00745,
        help = 'The step size of adapting threshold, defaut is 0.00745')
    parser.add_argument('--min_eps', type = float, default = 0.,
        help = 'The minimum adversarial budget considered, use the negative number, default is 0.')
    parser.add_argument('--max_eps', type = float, default = np.inf,
        help = 'The maximum adversarial budget considered, default is inf.')
    ...
    parser.add_argument('--mixed_loss', action = BooleanParser, default = True,
        help = 'Whether or not to use mixed loss, default is True.')
    ...
    # Parse the attacker
    train_attacker = parse_attacker(**args.attack)
    test_attacker = parse_attacker(**args.attack)
    default_threshold = train_attacker.threshold
    ...
    tosave = {'model_summary': str(model), 'setup_config': configs, 'train_loss': {}, 'train_acc': {}, 'train_acc_per_instance': {}, 'train_entropy_per_instance': {}, 'train_loss_per_instance': {}, 'train_idx2threshold': {},
    ...
    train_instancewise(model = model, train_loader = train_loader, valid_loader = valid_loader, test_loader = test_loader, train_attacker = train_attacker, test_attacker = test_attacker,
        epoch_num = args.epoch_num, epoch_ckpts = args.epoch_ckpts, optimizer = optimizer, lr_func = lr_func, eps_func = eps_func, mixed_loss = args.mixed_loss,
        out_folder = args.out_folder, model_name = args.model_name, device = device, criterion = criterion, tosave = tosave, gamma = args.gamma, min_eps = -args.min_eps, max_eps = args.max_eps, 
        beta = args.beta, warmup_epoch = args.warmup_epoch, default_threshold = default_threshold, idx2group = idx2group, feature_mode = args.feature_mode)
