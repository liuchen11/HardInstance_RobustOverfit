...
if __name__ == '__main__':
    ...
    parser.add_argument('--subset', action = DictParser, default = None,
        help = 'The type of the subset, default is None')
    ...
    train_loader, valid_loader, test_loader, classes = parse_data(name = args.dataset, batch_size = args.batch_size, valid_ratio = args.valid_ratio, train_subset = train_subset)
    ...
    train(model = model, train_loader = train_loader, valid_loader = valid_loader, test_loader = test_loader, train_attacker = train_attacker, test_attacker = test_attacker,
        epoch_num = args.epoch_num, epoch_ckpts = args.epoch_ckpts, optimizer = optimizer, lr_func = lr_func, eps_func = eps_func,
        out_folder = args.out_folder, model_name = args.model_name, device = device, criterion = criterion, tosave = tosave, idx2group = idx2group, feature_mode = args.feature_mode)
