...
class IndexedCIFAR10_C(torch.utils.data.Dataset):

    def __init__(self, root, c_type, level, transform = None, target_transform = None):

        super(IndexedCIFAR10_C, self).__init__()

        data_file = os.path.join(root, '%s.npy' % c_type)
        label_file = os.path.join(root, 'labels.npy')

        self.data_list = np.load(data_file)
        self.label_list = np.load(label_file).astype(np.int_)

        assert level in [1, 2, 3, 4, 5]

        self.data_list = self.data_list[10000 * (level - 1): 10000 * level]
        self.label_list = self.label_list[10000 * (level - 1): 10000 * level]
    ...
    def __getitem__(self, index):
        ...
        img, target = self.data_list[index], self.label_list[index]
...
def cifar10_c(batch_size, c_type, level):

    transform_test = transforms.Compose([
        transforms.ToTensor()
        ])

    testset = IndexedCIFAR10_C(root = './extradata/cifar10-c', c_type = c_type, level = level, transform = transform_test)

    classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')

    instance_num = len(testset)
    indices = list(range(instance_num))
...
def cifar10_plus(batch_size, plus_prop, valid_ratio, augmentation = True, train_subset = None):
    ...
    trainset = IndexedCIFAR10_Plus(root4ori = './data/cifar10', root4plus = './extradata/cifar10', train = True, download = True, transform = transform_train)
    validset = IndexedCIFAR10_Plus(root4ori = './data/cifar10', root4plus = './extradata/cifar10', train = True, download = True, transform = transform_valid)
    testset = IndexedCIFAR10(root = './data/cifar10', train = False, download = True, transform = transform_test)
